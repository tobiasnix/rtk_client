...
class NmeaParser:
    ...
    def _parse_gga(self, msg: pynmea2.types.talker.GGA) -> None:
        ...
        if msg.latitude is not None and msg.longitude is not None and new_fix_type > FIX_QUALITY_INVALID:
            ...
            updates["position"] = {"lat": msg.latitude, "lon": msg.longitude, "alt": alt_val}
    ...
    def _parse_gsv(self, msg: pynmea2.types.talker.GSV) -> None:
        ...
        for i in range(1, 5):
            ...
            try:
                elev = int(getattr(msg, elevation_key)) if getattr(msg, elevation_key) else None
            except (ValueError, TypeError):
                elev = None

            try:
                azim = int(getattr(msg, azimuth_key)) if getattr(msg, azimuth_key) else None
            except (ValueError, TypeError):
                azim = None

            try:
                snr = int(getattr(msg, snr_key)) if getattr(msg, snr_key) else 0  # Default SNR to 0 if missing
            except (ValueError, TypeError):
                snr = 0
        ...
        if is_last_sentence:
            ...
            if len(self._current_gsv_sequence_sats) != num_sv_in_view:
                logger.debug(
                    f"GSV mismatch: Header reported {num_sv_in_view} SVs, found {len(self._current_gsv_sequence_sats)} in sequence."
                )
            ...
            updates = {
                ...
                "max_satellites_seen": max(
                    self._state.get_state_snapshot().get("max_satellites_seen", 0), num_sv_in_view
                ),
