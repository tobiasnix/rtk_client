        ...
        while controller.is_running and not shutdown_requested:
                ...
                if key == curses.KEY_RESIZE:
                    ...
                    if key == ord("q") or key == ord("Q"):
                        ...
                        if controller and controller.state:
                            controller.state.add_ui_log_message("Shutdown requested by user (q).")
                        shutdown_requested = True
            ...
            # --- Exception Handling for the Loop Iteration ---
                ...
                if not shutdown_requested:
                    ...
                    if controller and controller.state:
                        controller.state.add_ui_log_message("Shutdown (Ctrl+C in loop).")
                    shutdown_requested = True
                break
            except curses.error as loop_curses_err:
                # Handle curses errors during getch() or update_display()
                logger.error(
                    f"Curses error in main loop iteration: {loop_curses_err}", exc_info=False
                )  # Keep log concise
                if status_display_obj:
                    status_display_obj.trigger_redraw()  # Try to recover display
                ...
                if controller and controller.state:
                    controller.state.add_ui_log_message(f"FATAL LOOP ERROR: {loop_err}")
