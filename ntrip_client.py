...
class NtripClient:
    ...
    def stop(self) -> None:
        ...
        if self._thread and self._thread.is_alive():
            ...
            if self._thread.is_alive():
                ...
                logger.warning(
                    f"NTRIP thread did not exit cleanly after {join_timeout}s. Current state: {self._connection_state.current_state}"
                )
    ...
    def _update_state_from_connection_state(self):
        ...
        if (
            conn_state.current_state == NtripConnectionState.DISCONNECTED
            and self._next_reconnect_time
            and not conn_state.has_given_up()
        ):
    ...
    def _connect(self) -> bool:
        ...
        connect_msg = (
            f"Connecting to {self._config.ntrip_server}:{self._config.ntrip_port}/{self._config.ntrip_mountpoint}..."
        )
        ...
        try:
            ...
            if self._config.ntrip_tls:
                ...
                self._socket = context.wrap_socket(self._socket, server_hostname=self._config.ntrip_server)
            ...
            if " 200 OK" in first_line or first_line.startswith("ICY 200 OK"):  # Handle ICY variant
                ...
                self._connection_state.set_state(
                    NtripConnectionState.DISCONNECTED, f"Failed ({current_attempts}): {status_line[:30]}"
                )
            ...
            current_attempts = self._connection_state.increment_reconnect_attempts()
            self._connection_state.set_state(
                NtripConnectionState.DISCONNECTED, f"Sock Err ({current_attempts}): {str(e)[:25]}"
            )
            ...
            current_attempts = self._connection_state.increment_reconnect_attempts()
            self._connection_state.set_state(
                NtripConnectionState.DISCONNECTED, f"Error ({current_attempts}): {str(e)[:25]}"
            )
            self._state.increment_error_count("ntrip")
            self._close_socket()
            return False
    ...
    def _run(self) -> None:
        ...
        while self._running.is_set():
            ...
            # --- Disconnected / Gave Up State Logic ---
                ...
                if should_reconnect:
                    logger.info(
                        f"Attempting NTRIP connection (Attempt: {self._connection_state.reconnect_attempts + 1})..."
                    )
                    if self._connect():  # connect() handles state changes and updates
                        ...
                        if self._check_retry_limit():
                            ...
                            logger.info(
                                f"NTRIP connection failed ({self._connection_state.reconnect_attempts}/{MAX_NTRIP_RETRIES}). Retrying in {wait_time:.1f} seconds."
                            )
                            # UI message handled by state update
